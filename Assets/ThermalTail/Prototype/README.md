...
At an edge the motor probes for the adjacent face, verifies both faces meet, then builds and sweeps a short arc with body clearance. Position and surface orientation advance together, preserving the movement frame and mouse heading. Unsupported edges, unmarked faces, gaps, and blocked arcs stop movement. This targets simple planar geometry, not arbitrary curved meshes or moving platforms. Dynamic obstruction during a committed turn pauses it until clear; checkpoint warps cancel it.

If the ordinary turn cannot fit, **Skip Narrow Surfaces** tries crossing one short adjoining face to reach the next clear face. This supports shallow ledges in both directions and thin caps without requiring a resting position on the intermediate face. **Maximum Skip Distance** defaults to **0.75 units**, measured along the skipped face. Reduce it to restrict traversal; zero or disabling Skip Narrow Surfaces disables this fallback. Automatic Corners must also be enabled. Each face must be marked climbable and connected, each turn must respect Maximum Corner Angle, and the entire body route must be clear. It does not jump gaps or chain across multiple intermediate faces.
